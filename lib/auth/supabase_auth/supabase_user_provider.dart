...
class DostaraSupabaseUser extends BaseAuthUser {
  DostaraSupabaseUser(this.user);
  User? user;
  @override
...
Stream<BaseAuthUser> dostaraSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = DostaraSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
